...
| I3 | **Build the tenon adapter module.** Six call sites into one; name by role; diagnostic ids opaque; GENE_DIRS to spec config. **Before tenon's renames land** — then T3/T5/T9 cost one file instead of six. **Landed:** `improve/tenon.py` holds every tenon call behind roles named `gate`/`identity`/`files`/`catalog`/`compile`/`drifted`/`dispatch`/`clean`, with one terminator reader, ids never branched on, `spec.genes` config, recorded-stream tests, and a confinement grep keeping the other modules out of the CLI's vocabulary. | M |
| I4 | **`iterate` in the adapter:** check -> apply -> pre-drift -> run -> post-drift, one record with `phase_failed`. Do the drifts **in-process against one loaded source** — at search scale two extra process launches plus two full projection regenerations per candidate is the dominant non-model cost. **Landed:** `iterate` composes the roles and records findings only, with the pre-run drift off by default because compile just enumerated what it wrote; collapsing the remaining two loads needs a tenon-side facility for two drift reports from one load, left as `TODO(I4-followup)`. | M |
