...
// issue #49 to the Agent Plugins 1.0 mcp.json server-entry vocabulary). Each
// mcp/<name>.md carries one closed YAML frontmatter document selecting
// exactly one target form: remote streamable-http, or installed — an exact
// operator-installed native-mcp capability (ADR 0014) selected by package
// and capability id. Load validates the installed frontmatter shape only;
// resolving the selection against the operator's integration store happens
// offline at generation time (internal/claude, internal/codex), exactly
// like plugin ${PLUGIN_DATA}.
//
...
// selection (ADR 0016, issue #49). Kind discriminates which fields apply.
...
// connectionRemoteFields and connectionInstalledFields are the recognized
// frontmatter fields for each target form, keyed by the "type" discriminator
// (issue #49): remote uses type: streamable-http and installed uses type:
// installed. Every other field is unknown for that form.
var (
...
// (issue #49): one plain field "type" that discriminates exactly one target
// form — "streamable-http" (url, optional headers) or "installed" (package,
// capability). "sse" fails as a deprecated, unsupported transport; "stdio"
// fails as not yet supported in authored files (repo-relative stdio is
// issue #50). It validates installed frontmatter shape only: package and
// capability must each be present, non-empty, and match ADR 0014's stable
// identifier grammar. It never contacts the integration store; that happens
// at generation time.
func parseConnection(content, path string, diags *diagnostics.List) (*Connection, bool) {
	...
	if !doc.Has("type") {
		diags.Errorf("mcp.frontmatter.missing", path,
			"frontmatter must carry the field \"type\" set to \"streamable-http\" or \"installed\"")
		return nil, false
	}
	...
	case "stdio":
		diags.Errorf("mcp.transport.invalid", path,
			"repo-relative stdio connections are not yet supported in authored files")
		return nil, false
	default:
		diags.Errorf("mcp.frontmatter.invalid", path,
			"frontmatter field \"type\" must be \"streamable-http\" or \"installed\"; found %q", typeVal)
		return nil, false
	}
...
// connectionHeaderValuePattern is the exact, falsifiable grammar an authored
// header value must satisfy when it carries a "$": an optional literal
// prefix containing no "$", followed by exactly one ${VAR} environment-
// variable reference, with nothing after it. VAR is captured for the
// PLUGIN_ROOT/PLUGIN_DATA denylist below. Any other use of "$" — two
// references, a reference followed by a suffix, or a lone "$" — fails.
var connectionHeaderValuePattern = regexp.MustCompile(`^[^$]*\$\{([A-Z_][A-Z0-9_]*)\}$`)

// deniedConnectionHeaderVars are the plugin-scoped variables an authored
// header may never reference: ${PLUGIN_ROOT} and ${PLUGIN_DATA} expand only
// inside a plugin's own mcp.json, against that plugin's own identity.
var deniedConnectionHeaderVars = map[string]bool{"PLUGIN_ROOT": true, "PLUGIN_DATA": true}
...
// the authored-header environment-variable-reference grammar: tenon never
// resolves an authored header value, so any embedded literal is entirely the
// author's responsibility, and any use of "$" outside the single-reference
// grammar is rejected outright rather than guessed at.
func validAuthoredHeaders(headers map[string]string) error {
	...
	for _, name := range sortedStringKeys(headers) {
		...
		m := connectionHeaderValuePattern.FindStringSubmatch(value)
		if m == nil {
			return fmt.Errorf("the header %q value may use \"$\" only as an optional literal prefix followed by exactly one ${VAR} environment-variable reference at the end", name)
		}
		if deniedConnectionHeaderVars[m[1]] {
			return fmt.Errorf("the header %q value may not reference ${%s}, which is reserved for plugin-scoped expansion", name, m[1])
		}
	}
	return nil
}
